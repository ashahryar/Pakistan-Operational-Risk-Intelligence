...
PORI is a batch-oriented data engineering pipeline, not a real-time streaming system. Apache Airflow triggers scheduled DAG runs that scrape government sources, parse the downloaded PDFs/HTML into structured records, validate those records against domain rules, and load them into PostgreSQL. From PostgreSQL, raw and parsed files are archived to Amazon S3, and the Streamlit dashboard reads directly from PostgreSQL to render operational views. A second-stage cloud analytics layer (AWS Glue → Amazon Redshift) exists in the codebase as a complete, ready-to-enable extension, but its Airflow tasks are currently commented out, so it does not run as part of any scheduled pipeline today.

The diagram below is the full architecture, covering both the active pipeline and the implemented-but-inactive cloud warehouse extension:

![Pakistan Operational Risk Intelligence Platform Architecture](docs/architecture.png)

The system is best understood as nine layers:

1. **Government Data Sources**
   - NDMA (National Disaster Management Authority) — sitreps, advisories, guidelines (PDF)
   - PDMA Punjab (Provincial Disaster Management Authority) — daily reports, rainfall reports, gauge reports, earthquake reports (PDF)
   - PMD (Pakistan Meteorological Department) — daily forecast, weekly outlook, weather alerts (HTML)
   - None of the three publish an official API; everything is scraped from public pages.

2. **Extraction Layer** (`scripts/extraction/`)
   - `requests` + `BeautifulSoup` scrapers with retry/backoff handling
   - Source-specific extractors: `extract_ndma.py`, `extract_pdma.py`, `extract_pmd.py`
   - Download de-duplication via a metadata JSON file, so already-downloaded PDFs aren't re-fetched

3. **Parsing Layer** (`scripts/parsing/`)
   - PDF table extraction via `pdfplumber`, `camelot-py`, `PyMuPDF`
   - HTML table/text parsing for PMD forecasts and alerts
   - Output: structured JSON/CSV records per source, written to `data/parsed/<source>/`

4. **Validation Layer** (`validation/`)
   - Per-source schema checks, completeness checks, and a numeric quality score (`ndma/`, `pdma/`, `pmd/`)
   - Shared field-level rules (e.g. valid temperature/humidity ranges) in `rules.py`
   - Invoked directly inside the parsers, so bad records are flagged before they ever reach PostgreSQL

5. **Orchestration** (`pipeline/dags/`)
   - Apache Airflow 2.9.3, `LocalExecutor`, running in Docker Compose
   - Seven DAGs covering per-source pipelines, a master trigger DAG, a weekly full pipeline, a manual DAG, and a backfill DAG
   - `retries=2–3`, 5-minute retry delay, `catchup=False`, `max_active_runs=1`, plus success/failure callbacks

6. **Operational Database**
   - PostgreSQL 15, one table per report type, covering disaster impact, weather, rainfall, and river/hydrology data
   - `UNIQUE` constraints on natural keys (report number/date/province, or station) make DAG re-runs idempotent

7. **Cloud Storage**
   - Amazon S3 archival of both raw and parsed files (`aws/s3/upload.py`), invoked from every DAG's `upload_raw` task
   - Idempotent uploads (skip files that already exist via a `HEAD` check)

8. **Cloud Analytics Layer — implemented but currently inactive**
   - AWS Glue ETL jobs (PySpark) that would read from S3 and write into Amazon Redshift
   - Amazon Redshift table DDL and connection setup
   - An AWS Lambda function that would route new S3 objects to the matching Glue job
   - **This entire layer is fully coded but its Airflow tasks are commented out in every DAG** — see [§4.2](#42-active-vs-implemented-but-inactive-architecture) and [§7](#7-aws-glue--redshift)

9. **Visualization Layer** (`dashboard/`)
   - Streamlit multi-page app, reading directly from PostgreSQL via `dashboard/db.py`
   - Plotly charts, a national overview page, and five domain-specific analytics pages (NDMA Casualties, NDMA Damage, PMD Weather, PDMA Rainfall, PDMA Rivers)
   - CSV/Excel export on every page; auto-refresh every 60 seconds

### 4.2 Active vs. Implemented-but-Inactive Architecture

This distinction matters, and the README documents it deliberately rather than implying a fully active cloud warehouse.

**ACTIVE PIPELINE** (what the Airflow scheduler actually runs today):
...
Apache Airflow orchestrates every step of this active flow — scheduling, retries, and success/failure callbacks.

**IMPLEMENTED BUT CURRENTLY INACTIVE** (complete, runnable code; not wired into any running DAG):
...
This is best described as an **implemented cloud warehouse extension** that is **currently inactive in the Airflow DAG task graph** — the Python/PySpark code, Redshift DDL, and Airflow helper functions (`start_glue_job`, `wait_for_glue_job`, Redshift verification) all exist and are already imported by the DAGs, but the actual `PythonOperator` tasks that would invoke them are commented out (see [§7](#7-aws-glue--redshift) for the exact code). PostgreSQL and S3 are the real, currently-running system of record; Glue and Redshift are a ready-to-enable extension, not a production-active component.

### 4.3 Architecture Diagram (Mermaid)

The Mermaid diagram below mirrors the image above: **solid arrows** mark the active, scheduled data flow; **dotted arrows** mark the implemented-but-inactive cloud warehouse extension.
...
flowchart LR
    ...
    LAMBDA["AWS Lambda\n(S3 trigger, implemented, not scheduled)"]
...
*(Solid arrows: active, scheduled data flow. Dotted arrows: implemented in code but disabled in the current DAG task graph, or Airflow's orchestration relationship to a task.)*
...
│   └── architecture.png         # Architecture diagram referenced in §4
│
